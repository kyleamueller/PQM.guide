...
syntax: >-
  BinaryFormat.List(binaryFormat as function, optional countOrCondition as any)
  as function
returnType: function
returnDescription: >-
  A binary format function that reads a sequence of items and returns them as a
  list.
parameters:
  - name: binaryFormat
    type: function
  ...
  - name: countOrCondition
    ...
    required: false
    description: >-
      The number of items to read, a function that returns true to continue or
      false to stop, or a binary format that reads the count preceding the list.
      If omitted, reads until end of data.
compatibility:
  pbiDesktop: true
  pbiService: true
  excelDesktop: true
