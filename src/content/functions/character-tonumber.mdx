---
title: "Character.ToNumber"
slug: "character-tonumber"
category: "Text"
description: "Returns the Unicode code point number for the given character."
syntax: "Character.ToNumber(character as text) as number"
returnType: "number"
returnDescription: "The numeric Unicode code point of the character."
parameters:
  - name: "character"
    type: "text"
    required: true
    description: "A single-character text value."
compatibility:
  pbiDesktop: true
  pbiService: true
  excelDesktop: true
  excelOnline: true
  dataflows: true
  fabricNotebooks: true
relatedFunctions:
  - "character-fromnumber"
  - "text-tolist"
  - "text-at"
---

## Remarks

`Character.ToNumber` returns the Unicode code point (an integer) for the given single character. This is the inverse of `Character.FromNumber`. The input must be exactly one character long — passing a multi-character string or an empty string raises an error.

The primary use of this function is character classification: you can test whether a character falls within a known Unicode range to determine if it is a digit (48–57), uppercase letter (65–90), lowercase letter (97–122), or a control character (0–31). This approach is faster than converting to a list and filtering when you only need to classify one character at a time.

When processing every character in a string, combine `Text.ToList` with `List.Transform` to apply `Character.ToNumber` across all characters at once. The resulting list of code points can then be filtered or analyzed with standard list functions.

Note that Power Query text comparisons are case-sensitive by default. If you need to classify characters without regard to case, compare against both the uppercase and lowercase ranges, or normalize the character with `Text.Upper` or `Text.Lower` first.

## Examples

### Example 1: Get the code point for a character

```powerquery
Character.ToNumber("A")
```

<!--output
{"columns":[{"name":"Result","type":"number"}],"rows":[{"Result":65}]}
-->

### Example 2: Check whether a character is a digit

Compare the code point against the digit range (48–57) to classify a character without using `Text.Contains` or regular expressions.

```powerquery
let
    ch = "7",
    code = Character.ToNumber(ch),
    isDigit = code >= 48 and code <= 57
in
    isDigit
```

<!--output
{"columns":[{"name":"Result","type":"logical"}],"rows":[{"Result":true}]}
-->
<!--steps
{
  "steps": [
    {
      "name": "ch",
      "description": "Defines the single character \"7\" as the input to classify.",
      "output": {"columns":[{"name":"Value","type":"text"}],"rows":[{"Value":"7"}]}
    },
    {
      "name": "code",
      "description": "Gets the Unicode code point for \"7\" using Character.ToNumber, which returns 55.",
      "output": {"columns":[{"name":"Value","type":"number"}],"rows":[{"Value":55}]}
    },
    {
      "name": "isDigit",
      "description": "Tests whether the code point (55) falls within the digit range 48–57, returning true.",
      "output": {"columns":[{"name":"Value","type":"logical"}],"rows":[{"Value":true}]}
    },
    {
      "name": "Result",
      "description": "The final output — a logical true, confirming that \"7\" is a digit character.",
      "output": {"columns":[{"name":"Result","type":"logical"}],"rows":[{"Result":true}]}
    }
  ]
}
-->

### Example 3: Get code points for each character in a string

Pair with `Text.ToList` to process all characters in a value, such as scanning employee IDs for non-alphanumeric characters.

```powerquery
List.Transform(Text.ToList("E001"), Character.ToNumber)
```

<!--output
{"columns":[{"name":"Result","type":"list"}],"rows":[{"Result":[69,48,48,49]}]}
-->
