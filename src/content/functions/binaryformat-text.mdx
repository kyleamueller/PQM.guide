...
parameters:
  - name: length
    ...
    required: true
    description: >-
      The number of bytes to decode, or a binary format that reads the length
      preceding the text.
  - name: encoding
    type: nullable number
    required: false
    description: >-
      The text encoding to use (e.g., TextEncoding.Ascii, TextEncoding.Utf8,
      TextEncoding.Utf16). If not specified, encoding is detected from Unicode
      byte order marks, defaulting to UTF-8.
compatibility:
  pbiDesktop: true
  pbiService: true
  excelDesktop: true
