---
title: "Table.Contains"
slug: "table-contains"
category: "Table"
description: "Returns true if the table contains a row matching the given record."
syntax: "Table.Contains(table as table, row as record, optional equationCriteria as any) as logical"
returnType: "logical"
returnDescription: "true if the table contains a matching row; false otherwise."
parameters:
  - name: "table"
    type: "table"
    required: true
    description: "The table to search."
  - name: "row"
    type: "record"
    required: true
    description: "A record whose fields are matched against table rows."
  - name: "equationCriteria"
    type: "any"
    required: false
    description: "Optional. A column name, list of column names, or comparer function to control which fields are compared."
compatibility:
  pbiDesktop: true
  pbiService: true
  excelDesktop: true
  excelOnline: true
  dataflows: true
  fabricNotebooks: true
relatedFunctions:
  - "table-containsall"
  - "table-containsany"
  - "table-isdistinct"
  - "table-selectrows"
---

## Remarks

`Table.Contains` checks whether a table includes at least one row matching the provided record. By default, every field in the record is compared against the corresponding column. The optional `equationCriteria` parameter can restrict the comparison to specific columns, which is useful when you only care about partial matches (e.g., checking that a CustomerID exists regardless of other column values).

The `equationCriteria` parameter accepts a column name (text), a list of column names, or a comparer function. When restricting to specific columns, the record passed as `row` only needs to include fields for those columns — extra fields in the record are ignored.

For checking multiple rows at once, use `Table.ContainsAll` (all must be present) or `Table.ContainsAny` (at least one must be present). For retrieving matching rows rather than a boolean result, use `Table.SelectRows`. Note that `Table.Contains` does not fold to data sources, so for large source tables prefer `Table.SelectRows` with a count check when folding matters.

## Examples

### Example 1: Check whether a specific customer has placed an order

```powerquery
let
    Sales = #table(
        {"OrderID", "CustomerName", "Product"},
        {{1,"Alice","Widget A"},{2,"Bob","Gadget B"},{3,"Charlie","Widget C"}}
    )
in
    Table.Contains(Sales, [CustomerName = "Bob"], "CustomerName")
```

<!--output
{"columns":[{"name":"Result","type":"logical"}],"rows":[{"Result":"true"}]}
-->

### Example 2: Check for an exact row match (all fields)

Both the CustomerName and Product must match.

```powerquery
let
    Sales = #table(
        {"CustomerName", "Product"},
        {{"Alice","Widget A"},{"Bob","Gadget B"},{"Diana","Widget A"}}
    )
in
    Table.Contains(Sales, [CustomerName = "Alice", Product = "Gadget B"])
```

<!--output
{"columns":[{"name":"Result","type":"logical"}],"rows":[{"Result":"false"}]}
-->

### Example 3: Guard a lookup before accessing a record

Use `Table.Contains` to prevent an error when a key may not exist in the lookup table.

```powerquery
let
    Products = #table(
        {"ProductID", "ProductName", "Price"},
        {{1,"Widget A",25.00},{2,"Gadget B",50.00},{3,"Widget C",15.00}}
    ),
    SearchID = 99,
    Exists   = Table.Contains(Products, [ProductID = SearchID], "ProductID"),
    Result   = if Exists
               then Table.First(Table.SelectRows(Products, each [ProductID] = SearchID))[ProductName]
               else "Product not found"
in
    Result
```

<!--output
{"columns":[{"name":"Result","type":"text"}],"rows":[{"Result":"Product not found"}]}
-->
<!--steps
{
  "steps": [
    {
      "name": "Products",
      "description": "Creates an inline table with ProductID, ProductName, and Price columns for three products.",
      "output": {"columns":[{"name":"ProductID","type":"any"},{"name":"ProductName","type":"any"},{"name":"Price","type":"any"}],"rows":[{"ProductID":"1","ProductName":"Widget A","Price":"25"},{"ProductID":"2","ProductName":"Gadget B","Price":"50"},{"ProductID":"3","ProductName":"Widget C","Price":"15"}]}
    },
    {
      "name": "SearchID",
      "description": "Stores the product ID to look up (99) as a number.",
      "output": {"columns":[{"name":"Value","type":"number"}],"rows":[{"Value":"99"}]}
    },
    {
      "name": "Exists",
      "description": "Checks whether any row in Products has a ProductID matching SearchID (99), returning false since no such product exists.",
      "output": {"columns":[{"name":"Value","type":"logical"}],"rows":[{"Value":"false"}]}
    },
    {
      "name": "Result",
      "description": "The final output — returns the ProductName if Exists is true, or \"Product not found\" if false.",
      "output": {"columns":[{"name":"Result","type":"text"}],"rows":[{"Result":"Product not found"}]}
    }
  ]
}
-->
