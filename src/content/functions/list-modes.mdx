---
title: "List.Modes"
slug: "list-modes"
category: "List"
description: "Returns all values that appear with the highest frequency in the list (all modes when there is a tie)."
syntax: "List.Modes(list as list, optional equationCriteria as any) as list"
returnType: "list"
returnDescription: "A list of all values tied for the highest frequency."
parameters:
  - name: "list"
    type: "list"
    required: true
    description: "The list to find all modes of."
  - name: "equationCriteria"
    type: "any"
    required: false
    description: "An optional equation criteria to control how equality is determined."
compatibility:
  pbiDesktop: true
  pbiService: true
  excelDesktop: true
  excelOnline: true
  dataflows: true
  fabricNotebooks: true
relatedFunctions:
  - "list-mode"
  - "list-median"
  - "list-distinct"
  - "list-count"
---

## Remarks

`List.Modes` returns a **list** of all values that share the maximum frequency in the input list. When there is only one most-frequent value, it returns a single-element list. When multiple values tie for the highest frequency, all tied values are returned.

This is the multi-value counterpart of `List.Mode`. Use `List.Modes` when you need to detect whether a dataset is multimodal (has ties) or when downstream logic must handle all equally common values. If you only need one mode and ties are not a concern, `List.Mode` returns a scalar directly, which is simpler to use.

The order of modes in the result follows their first appearance in the source list. Null values count as occurrences and can be included in the result. An empty list causes an error — guard with `List.IsEmpty` if needed. Comparison is case-sensitive for text by default; pass `Comparer.OrdinalIgnoreCase` as the optional `equationCriteria` argument for case-insensitive frequency grouping.

## Examples

### Example 1: Single mode

```powerquery
List.Modes({1, 2, 2, 3, 3, 3, 4})
```

<!--output
{"columns":[{"name":"Result","type":"any"}],"rows":[{"Result":"{3}"}]}
-->

### Example 2: Multiple tied modes

```powerquery
List.Modes({1, 2, 2, 3, 3, 4})
```

<!--output
{"columns":[{"name":"Result","type":"any"}],"rows":[{"Result":"{2, 3}"}]}
-->

### Example 3: Detect tied categories

```powerquery
let
    Categories = {"A", "B", "A", "B", "C"},
    AllModes = List.Modes(Categories)
in
    AllModes
```

<!--output
{"columns":[{"name":"Result","type":"any"}],"rows":[{"Result":"{A, B}"}]}
-->

<!--steps
{
  "steps": [
    {
      "name": "Categories",
      "description": "Defines a list of category labels, where \"A\" and \"B\" each appear twice and \"C\" appears once.",
      "output": {"columns":[{"name":"Value","type":"text"}],"rows":[{"Value":"A"},{"Value":"B"},{"Value":"A"},{"Value":"B"},{"Value":"C"}]}
    },
    {
      "name": "AllModes",
      "description": "Finds all values tied for the highest frequency in Categories — both \"A\" and \"B\" appear twice, so both are returned.",
      "output": {"columns":[{"name":"Result","type":"any"}],"rows":[{"Result":"{A, B}"}]}
    },
    {
      "name": "Result",
      "description": "The final output — a list of all tied mode values from the Categories list.",
      "output": {"columns":[{"name":"Result","type":"any"}],"rows":[{"Result":"{A, B}"}]}
    }
  ]
}
-->
