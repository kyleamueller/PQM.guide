---
title: "List.RemoveMatchingItems"
slug: "list-removematchingitems"
category: "List"
description: "Removes all occurrences of items found in the second list from the first list."
syntax: "List.RemoveMatchingItems(list1 as list, list2 as list, optional equationCriteria as any) as list"
returnType: "list"
returnDescription: "A new list with all occurrences of list2 items removed from list1."
parameters:
  - name: "list1"
    type: "list"
    required: true
    description: "The source list to remove items from."
  - name: "list2"
    type: "list"
    required: true
    description: "The list of values to remove from list1."
  - name: "equationCriteria"
    type: "any"
    required: false
    description: "An optional equation criteria to control how equality is determined."
compatibility:
  pbiDesktop: true
  pbiService: true
  excelDesktop: true
  excelOnline: true
  dataflows: true
  fabricNotebooks: true
relatedFunctions:
  - "list-removeitems"
  - "list-difference"
  - "list-removenulls"
  - "list-select"
---

## Remarks

`List.RemoveMatchingItems` removes all occurrences of any value in `list2` from `list1`. It scans through `list1` and filters out any item that appears anywhere in `list2`. Duplicates in `list1` are both removed if the value appears in `list2`; duplicates in `list1` that are not in `list2` are preserved.

The key distinction from related functions: `List.RemoveMatchingItems` removes **every occurrence** of each matching value; `List.RemoveItems` removes only one occurrence per entry in `list2` (one-for-one removal); `List.Difference` performs a set-difference and deduplicates the result. Choose based on whether you want value-based filtering (this function), occurrence counting (`List.RemoveItems`), or a set operation (`List.Difference`).

Comparison is case-sensitive for text by default. Pass `Comparer.OrdinalIgnoreCase` as the `equationCriteria` argument for case-insensitive removal. Returns a new list; the originals are not modified.

## Examples

### Example 1: Remove all matching values

```powerquery
List.RemoveMatchingItems({1, 2, 3, 2, 4, 2, 5}, {2, 4})
```

<!--output
{"columns":[{"name":"Result","type":"any"}],"rows":[{"Result":"{1, 3, 5}"}]}
-->

### Example 2: Remove stopwords from a word list

```powerquery
let
    Words = {"the", "quick", "the", "brown", "fox", "the"},
    Stopwords = {"the", "a", "an"},
    Filtered = List.RemoveMatchingItems(Words, Stopwords)
in
    Filtered
```

<!--output
{"columns":[{"name":"Result","type":"any"}],"rows":[{"Result":"{quick, brown, fox}"}]}
-->

<!--steps
{
  "steps": [
    {
      "name": "Words",
      "description": "Defines a list of 6 words including multiple occurrences of the stopword \"the\".",
      "output": {"columns":[{"name":"Value","type":"text"}],"rows":[{"Value":"the"},{"Value":"quick"},{"Value":"the"},{"Value":"brown"},{"Value":"fox"},{"Value":"the"}]}
    },
    {
      "name": "Stopwords",
      "description": "Defines a list of common stopwords to be removed: \"the\", \"a\", and \"an\".",
      "output": {"columns":[{"name":"Value","type":"text"}],"rows":[{"Value":"the"},{"Value":"a"},{"Value":"an"}]}
    },
    {
      "name": "Filtered",
      "description": "Removes all occurrences of any Stopwords value from Words, eliminating all three instances of \"the\".",
      "output": {"columns":[{"name":"Result","type":"any"}],"rows":[{"Result":"{quick, brown, fox}"}]}
    },
    {
      "name": "Result",
      "description": "The final output — the word list with all stopword occurrences removed.",
      "output": {"columns":[{"name":"Result","type":"any"}],"rows":[{"Result":"{quick, brown, fox}"}]}
    }
  ]
}
-->

### Example 3: Case-insensitive removal

```powerquery
List.RemoveMatchingItems({"Apple", "APPLE", "Banana", "apple"}, {"apple"}, Comparer.OrdinalIgnoreCase)
```

<!--output
{"columns":[{"name":"Result","type":"any"}],"rows":[{"Result":"{Banana}"}]}
-->
