...
category: Table
description: >-
  Adds a column to the cube containing the results of a measure applied in each
  row's context.
syntax: >-
  Cube.AddMeasureColumn(cube as table, column as text, measureSelector as any)
  as table
returnType: table
returnDescription: >-
  The cube table with a new column containing measure values evaluated in the
  row context of each row.
parameters:
  - name: cube
    type: table
    required: true
