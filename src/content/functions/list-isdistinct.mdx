---
title: "List.IsDistinct"
slug: "list-isdistinct"
category: "List"
description: "Returns true if the list contains no duplicate values."
syntax: "List.IsDistinct(list as list, optional equationCriteria as any) as logical"
returnType: "logical"
returnDescription: "True if all values in the list are unique; false if any duplicates exist."
parameters:
  - name: "list"
    type: "list"
    required: true
    description: "The list to check for uniqueness."
  - name: "equationCriteria"
    type: "any"
    required: false
    description: "An optional equation criteria to control how equality is determined."
compatibility:
  pbiDesktop: true
  pbiService: true
  excelDesktop: true
  excelOnline: true
  dataflows: true
  fabricNotebooks: true
relatedFunctions:
  - "list-distinct"
  - "list-count"
  - "list-isempty"
---

## Remarks

`List.IsDistinct` checks whether every item in a list is unique. It returns `true` if all items are distinct, and `false` if any value appears more than once. This is a convenient data quality check — for example, verifying that a key column has no duplicate values before using it in a merge.

An empty list returns `true` (vacuous truth: no duplicates can exist). Null values are treated as equal to each other, so a list containing two or more nulls is not distinct. Comparison is case-sensitive for text by default; pass `Comparer.OrdinalIgnoreCase` for case-insensitive uniqueness checking.

`List.IsDistinct` is equivalent to `List.Count(list) = List.Count(List.Distinct(list))` but expresses intent more clearly. When you need to validate uniqueness for a table column, pass the column as a list: `List.IsDistinct(Table[ColumnName])`.

## Examples

### Example 1: All values are distinct

```powerquery
List.IsDistinct({1, 2, 3, 4, 5})
```

<!--output
{"columns":[{"name":"Result","type":"logical"}],"rows":[{"Result":"true"}]}
-->

### Example 2: Duplicate values present

```powerquery
List.IsDistinct({1, 2, 3, 2, 5})
```

<!--output
{"columns":[{"name":"Result","type":"logical"}],"rows":[{"Result":"false"}]}
-->

### Example 3: Case-insensitive check

```powerquery
List.IsDistinct({"Apple", "apple", "Banana"}, Comparer.OrdinalIgnoreCase)
```

<!--output
{"columns":[{"name":"Result","type":"logical"}],"rows":[{"Result":"false"}]}
-->

### Example 4: Validate a key column has no duplicates

```powerquery
let
    Keys = {"ID-001", "ID-002", "ID-003", "ID-004"},
    IsUnique = List.IsDistinct(Keys)
in
    IsUnique
```

<!--output
{"columns":[{"name":"Result","type":"logical"}],"rows":[{"Result":"true"}]}
-->

<!--steps
{
  "steps": [
    {
      "name": "Keys",
      "description": "Defines a list of key strings to validate for uniqueness.",
      "output": {"columns":[{"name":"Value","type":"text"}],"rows":[{"Value":"ID-001"},{"Value":"ID-002"},{"Value":"ID-003"},{"Value":"ID-004"}]}
    },
    {
      "name": "IsUnique",
      "description": "The final output — checks whether all values in the Keys list are distinct, returning true because no duplicates exist.",
      "output": {"columns":[{"name":"Result","type":"logical"}],"rows":[{"Result":"true"}]}
    }
  ]
}
-->
