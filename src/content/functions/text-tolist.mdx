---
title: "Text.ToList"
slug: "text-tolist"
category: "Text"
description: "Returns a list of single-character text values, one for each character in the text."
syntax: "Text.ToList(text as text) as list"
returnType: "list"
returnDescription: "A list of single-character text values representing each character in text."
parameters:
  - name: "text"
    type: "text"
    required: true
    description: "The text value to convert to a list of characters."
compatibility:
  pbiDesktop: true
  pbiService: true
  excelDesktop: true
  excelOnline: true
  dataflows: true
  fabricNotebooks: true
relatedFunctions:
  - "text-at"
  - "text-split"
  - "text-combine"
  - "text-positionofany"
  - "text-select"
  - "text-remove"
---

## Remarks

`Text.ToList` splits a text value into a list of single-character elements, preserving the original order. It is the standard way to perform character-level operations in Power Query — iterating over every character in a string using list functions.

The resulting list can be processed with any list function: `List.Select` to filter characters, `List.Transform` to map over them (e.g., with `Character.ToNumber`), `List.Count` to count occurrences, or `List.Accumulate` for stateful processing. To reconstruct text from the list, use `Text.Combine`.

`Text.ToList` is more expressive than `Text.Split` for character-level work because it makes the intent clear and does not require a separator argument. It is equivalent to `Text.Split` only when using a delimiter that never appears in the string.

Note that `Text.ToList` operates on Unicode code units, not grapheme clusters. Characters that consist of multiple code units (such as certain emoji or composed diacritics) may appear as multiple list entries.

## Examples

### Example 1: Split a string into individual characters

```powerquery
Text.ToList("E001")
```

<!--output
{"columns":[{"name":"Result","type":"list"}],"rows":[{"Result":["E","0","0","1"]}]}
-->

### Example 2: Count occurrences of a character

Count how many digits appear in a mixed alphanumeric string by converting each character to its code point.

```powerquery
let
    chars = Text.ToList("alice smith"),
    spaces = List.Select(chars, each _ = " ")
in
    List.Count(spaces)
```

<!--output
{"columns":[{"name":"Result","type":"number"}],"rows":[{"Result":1}]}
-->

<!--steps
{
  "steps": [
    {
      "name": "chars",
      "description": "Converts the text \"alice smith\" into a list of individual characters using Text.ToList.",
      "output": {"columns":[{"name":"Value","type":"text"}],"rows":[{"Value":"a"},{"Value":"l"},{"Value":"i"},{"Value":"c"},{"Value":"e"},{"Value":" "},{"Value":"s"},{"Value":"m"},{"Value":"i"},{"Value":"t"},{"Value":"h"}]}
    },
    {
      "name": "spaces",
      "description": "The final output — filters the character list to keep only space characters, then counts them to find the number of spaces.",
      "output": {"columns":[{"name":"Result","type":"number"}],"rows":[{"Result":1}]}
    }
  ]
}
-->

### Example 3: Get distinct characters in a string

```powerquery
List.Distinct(Text.ToList("Engineering"))
```

<!--output
{"columns":[{"name":"Result","type":"list"}],"rows":[{"Result":["E","n","g","i","e","r","i","n","g"]}]}
-->

### Example 4: Transform and rebuild — uppercase each character individually

```powerquery
Text.Combine(List.Transform(Text.ToList("alice"), Text.Upper))
```

<!--output
{"columns":[{"name":"Result","type":"text"}],"rows":[{"Result":"ALICE"}]}
-->
