---
title: "List.RemoveNulls"
slug: "list-removenulls"
category: "List"
description: "Removes all null values from a list, returning only non-null items."
syntax: "List.RemoveNulls(list as list) as list"
returnType: "list"
returnDescription: "A new list containing only the non-null items from the original list."
parameters:
  - name: "list"
    type: "list"
    required: true
    description: "The list from which null values should be removed."
compatibility:
  pbiDesktop: true
  pbiService: true
  excelDesktop: true
  excelOnline: true
  dataflows: true
  fabricNotebooks: true
relatedFunctions:
  - "list-nonnullcount"
  - "list-select"
  - "list-removematchingitems"
  - "list-isempty"
---

## Remarks

`List.RemoveNulls` filters out all `null` values from a list, returning only non-null items. It is equivalent to `List.Select(list, each _ <> null)` but expresses intent more clearly.

This function is commonly used to clean a list before aggregating it, to prevent null propagation in calculations, or to filter the results of a lookup that returns `null` for unmatched items. For example, after using `List.Transform` to do a lookup, the resulting list may contain nulls for unmatched rows — `List.RemoveNulls` strips those out before passing to `List.Sum` or similar.

Non-null values including `false`, `0`, and `""` (empty string) are retained. Returns an empty list if all items are null. The original list is not modified. For counting how many items remain after removing nulls, use `List.NonNullCount` directly instead of removing nulls and counting.

## Examples

### Example 1: Remove null values from a mixed list

```powerquery
List.RemoveNulls({1, null, 3, null, 5})
```

<!--output
{"columns":[{"name":"Result","type":"any"}],"rows":[{"Result":"{1, 3, 5}"}]}
-->

### Example 2: All nulls removed leaves an empty list

```powerquery
List.RemoveNulls({null, null, null})
```

<!--output
{"columns":[{"name":"Result","type":"any"}],"rows":[{"Result":"{}"}]}
-->

### Example 3: Clean lookup results before summing

```powerquery
let
    Lookups = {100, null, 250, null, 400},
    CleanValues = List.RemoveNulls(Lookups),
    Total = List.Sum(CleanValues)
in
    Total
```

<!--output
{"columns":[{"name":"Result","type":"number"}],"rows":[{"Result":750}]}
-->

<!--steps
{
  "steps": [
    {
      "name": "Lookups",
      "description": "Defines a list of 5 lookup results containing 3 numeric values and 2 nulls representing unmatched rows.",
      "output": {"columns":[{"name":"Value","type":"any"}],"rows":[{"Value":100},{"Value":null},{"Value":250},{"Value":null},{"Value":400}]}
    },
    {
      "name": "CleanValues",
      "description": "Removes the 2 null values from Lookups, leaving only the 3 valid numbers: 100, 250, and 400.",
      "output": {"columns":[{"name":"Value","type":"number"}],"rows":[{"Value":100},{"Value":250},{"Value":400}]}
    },
    {
      "name": "Total",
      "description": "Sums all values in CleanValues, returning 750 (100 + 250 + 400).",
      "output": {"columns":[{"name":"Value","type":"number"}],"rows":[{"Value":750}]}
    },
    {
      "name": "Result",
      "description": "The final output — the sum of the non-null lookup values.",
      "output": {"columns":[{"name":"Result","type":"number"}],"rows":[{"Result":750}]}
    }
  ]
}
-->
