---
title: "List.RemoveRange"
slug: "list-removerange"
category: "List"
description: "Removes a specified number of items from a list starting at a given index position."
syntax: "List.RemoveRange(list as list, index as number, optional count as nullable number) as list"
returnType: "list"
returnDescription: "A new list with the specified range of items removed."
parameters:
  - name: "list"
    type: "list"
    required: true
    description: "The source list."
  - name: "index"
    type: "number"
    required: true
    description: "The zero-based index at which to start removing items."
  - name: "count"
    type: "nullable number"
    required: false
    description: "The number of items to remove. Defaults to 1 if not specified."
compatibility:
  pbiDesktop: true
  pbiService: true
  excelDesktop: true
  excelOnline: true
  dataflows: true
  fabricNotebooks: true
relatedFunctions:
  - "list-insertrange"
  - "list-replacerange"
  - "list-range"
  - "list-removefirstn"
  - "list-removelastn"
---

## Remarks

`List.RemoveRange` removes a contiguous block of items from a list by position, returning a new list without the removed items. The `index` is zero-based and specifies where removal starts; `count` specifies how many items to remove. If `count` is omitted, a single item is removed.

An `index` of `0` starts removal from the first item. If `count` extends beyond the end of the list, all items from `index` onward are removed — no error is raised for over-counting. The original list is not modified.

This is a position-based removal function. For value-based removal (remove items matching a certain value), use `List.RemoveMatchingItems` or `List.Select`. For removing from the front, `List.RemoveFirstN` is more readable. For replacing a range rather than just removing it, use `List.ReplaceRange`.

## Examples

### Example 1: Remove 2 items starting at index 1

```powerquery
List.RemoveRange({1, 2, 3, 4, 5}, 1, 2)
```

<!--output
{"columns":[{"name":"Result","type":"any"}],"rows":[{"Result":"{1, 4, 5}"}]}
-->

### Example 2: Remove a single item (default count)

```powerquery
List.RemoveRange({"A", "B", "C", "D"}, 2)
```

<!--output
{"columns":[{"name":"Result","type":"any"}],"rows":[{"Result":"{A, B, D}"}]}
-->

### Example 3: Remove from the middle of a list

```powerquery
let
    Months = {"Jan", "Feb", "Mar", "Apr", "May", "Jun"},
    WithoutQ2 = List.RemoveRange(Months, 3, 3)
in
    WithoutQ2
```

<!--output
{"columns":[{"name":"Result","type":"any"}],"rows":[{"Result":"{Jan, Feb, Mar}"}]}
-->

<!--steps
{
  "steps": [
    {
      "name": "Months",
      "description": "Defines a list of the first 6 month abbreviations, covering Q1 and Q2.",
      "output": {"columns":[{"name":"Value","type":"text"}],"rows":[{"Value":"Jan"},{"Value":"Feb"},{"Value":"Mar"},{"Value":"Apr"},{"Value":"May"},{"Value":"Jun"}]}
    },
    {
      "name": "WithoutQ2",
      "description": "Removes 3 items starting at index 3 (Apr, May, Jun) from Months, leaving only the Q1 months.",
      "output": {"columns":[{"name":"Result","type":"any"}],"rows":[{"Result":"{Jan, Feb, Mar}"}]}
    },
    {
      "name": "Result",
      "description": "The final output — the list with Q2 months removed, containing only Jan, Feb, and Mar.",
      "output": {"columns":[{"name":"Result","type":"any"}],"rows":[{"Result":"{Jan, Feb, Mar}"}]}
    }
  ]
}
-->
