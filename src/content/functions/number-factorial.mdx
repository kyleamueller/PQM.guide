---
title: "Number.Factorial"
slug: "number-factorial"
category: "Number"
description: "Returns the factorial of a non-negative integer (n! = 1 × 2 × ... × n)."
syntax: "Number.Factorial(number as number) as number"
returnType: "number"
returnDescription: "The factorial of the given non-negative integer."
parameters:
  - name: "number"
    type: "number"
    required: true
    description: "A non-negative integer whose factorial is computed. Must be a whole number >= 0."
compatibility:
  pbiDesktop: true
  pbiService: true
  excelDesktop: true
  excelOnline: true
  dataflows: true
  fabricNotebooks: true
relatedFunctions:
  - "number-combinations"
  - "number-permutations"
  - "number-power"
---

## Remarks

`Number.Factorial` computes n! — the product of all positive integers from 1 up to n. By convention, `0! = 1` (the empty product). The input must be a non-negative integer; passing a negative number or a fractional value raises an error.

Factorial values grow **extremely rapidly**: 10! = 3,628,800; 20! ≈ 2.4 × 10^18; 170! ≈ 7.3 × 10^306 (near the double-precision limit). `Number.Factorial(171)` returns infinity because the result overflows IEEE 754 double-precision. Do not use factorial to directly compute large combinatorics — use `Number.Combinations` or `Number.Permutations` instead, as those functions are implemented to avoid the intermediate factorial overflow.

Factorials are the mathematical foundation for: combinatorics (C(n,k) = n! / (k!(n-k)!)), permutations (P(n,k) = n! / (n-k)!), and probability distributions (Poisson, binomial). In Power Query they are most useful for small n in lookup tables, educational examples, or computing factorial-based normalizing constants.

## Examples

### Example 1: Factorial of 5

5! = 1 × 2 × 3 × 4 × 5 = 120

```powerquery
Number.Factorial(5)
```

<!--output
{"columns":[{"name":"Result","type":"number"}],"rows":[{"Result":120}]}
-->

### Example 2: Factorial of 0 — by convention equals 1

```powerquery
Number.Factorial(0)
```

<!--output
{"columns":[{"name":"Result","type":"number"}],"rows":[{"Result":1}]}
-->

### Example 3: Verify the combinations formula using factorials

C(5,2) = 5! / (2! × 3!) = 120 / (2 × 6) = 10. This confirms `Number.Combinations(5, 2)`.

```powerquery
let
    N = 5,
    K = 2,
    Computed = Number.Factorial(N) / (Number.Factorial(K) * Number.Factorial(N - K))
in
    Computed
```

<!--output
{"columns":[{"name":"Result","type":"number"}],"rows":[{"Result":10}]}
-->
<!--steps
{
  "steps": [
    {
      "name": "N",
      "description": "Assigns the total number of items to choose from (n = 5).",
      "output": {"columns":[{"name":"Value","type":"number"}],"rows":[{"Value":5}]}
    },
    {
      "name": "K",
      "description": "Assigns the number of items to select (k = 2).",
      "output": {"columns":[{"name":"Value","type":"number"}],"rows":[{"Value":2}]}
    },
    {
      "name": "Computed",
      "description": "Evaluates the combinations formula: Number.Factorial(5) / (Number.Factorial(2) × Number.Factorial(3)) = 120 / (2 × 6) = 10.",
      "output": {"columns":[{"name":"Value","type":"number"}],"rows":[{"Value":10}]}
    },
    {
      "name": "Result",
      "description": "The final output — C(5,2) = 10, confirming there are 10 ways to choose 2 items from 5.",
      "output": {"columns":[{"name":"Result","type":"number"}],"rows":[{"Result":10}]}
    }
  ]
}
-->
