...
Dataflows Gen2 is the most capable Power Query environment as of 2026. It runs in the cloud on the Modern Evaluator engine (rebuilt on .NET 8, **generally available as of February 2026**), supports 80+ connectors at GA, and stores results as Delta tables in OneLake. Output destinations include Fabric Lakehouse (Tables and Files), Fabric Warehouse, Fabric SQL database, Azure SQL, Azure Data Explorer (Kusto), Azure Data Lake Storage Gen2, Fabric KQL database, SharePoint Files, and Snowflake.

Requires a Fabric capacity (not just Power BI Pro). Supports on-premises sources via gateway.

At FabCon Atlanta 2026, Microsoft announced that **Variable Library integration is now generally available** in Dataflow Gen2, enabling enterprise-grade CI/CD and ALM patterns through centrally managed configuration. Additional improvements include enhanced SharePoint destination selection and Recent Tables in Modern Get Data for faster re-connection to previously used sources. Separately, at Build 2026 Microsoft announced that Azure Data Factory's **Mapping Data Flow transformations are now available inside Dataflow Gen2**, bringing low-code Spark-scale data engineering into the modern Fabric authoring experience.
...
### Power BI Service — Dataflows Gen1 (Legacy)

> **Retirement notice:** Microsoft has announced Dataflows Gen1 is moving to **Legacy** status. No new features will be developed. Existing dataflows continue to work for now, but Microsoft is finalizing retirement dates and will provide at least 12 months' notice before any capacity-level shutdown. Migration is strongly recommended. See the [official announcement](https://powerbi.microsoft.com/en-us/blog/dataflows-thank-you-for-eight-years-of-gen1-and-why-gen2-is-the-future/).
...
Dataflows Gen1 **does not support** the new executeQuery API — that is exclusive to Fabric Dataflows Gen2. To migrate, use the **Save as Dataflow Gen2** feature in the product, or the Save As API for bulk/CI-CD workflows.
...
**As of January 2026, the full Power Query experience — including the import wizard and Power Query Editor — is generally available in Excel for the web** for Microsoft 365 Business and Enterprise subscribers. Viewing and refreshing existing queries remains available to all Microsoft 365 subscribers (including Personal and Family plans); authoring and editing require Business or Enterprise.

Supported sources at GA include Excel Workbook, Text/CSV, XML, JSON, SQL Server Database, SharePoint Online List, OData, Blank Table, and Blank Query. Local file sources (from the user's desktop) remain unavailable in the web editor, which connects only to cloud and network sources. For very large datasets or complex multi-step pipelines, the desktop editor remains a better fit for performance reasons.
...
- **Cloud M engine (UI-driven):** Fabric Dataflows Gen2, Power BI Dataflows Gen1 *(Legacy — migration recommended)*, Excel Online, Power Automate/Apps Dataflows
