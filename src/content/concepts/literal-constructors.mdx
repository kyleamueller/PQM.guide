...
**`#time(hour, minute, second)`** — produces a `time`
```powerquery
#time(14, 30, 0)
```

**`#datetime(year, month, day, hour, minute, second)`** — produces a `datetime`
```powerquery
#datetime(2024, 3, 15, 14, 30, 0)
```

**`#datetimezone(year, month, day, hour, minute, second, offsetHours, offsetMinutes)`** — produces a `datetimezone`
```powerquery
#datetimezone(2024, 3, 15, 14, 30, 0, -5, 0)
```

**`#duration(days, hours, minutes, seconds)`** — produces a `duration`
```powerquery
#duration(1, 2, 30, 0)
```

**`#table(columnNames, rows)`** — produces a `table`
```powerquery
#table({"Name", "Score"}, {{"Alice", 95}, {"Bob", 82}})
```

**`#binary(bytes)`** — produces a `binary`
```powerquery
